...
// now for the substition formula for replacing the variable in an
// atom/predicate
void substitute(Literal<Term>& x, Literal<Term>& y, sub_list& z, int ix) {
    auto arg = (y.args).at(ix);

    // write the substitution to z
    get<sub_list_type>(z)[get<Variable>((x.args).at(ix)).name] = arg;
    (x.args).insert(x.args.begin() + ix, arg);
    ix++;
...
sub_list unify(Literal<Term> x, Literal<Term> y, sub_list z) {
    ...
    if ((x.args).size() != (y.args).size()) {
        z = "Error: Predicates have different numbers of arguments. "
            "Unification not possible.";
        return z; // need to make it return the substitution list with a fail
    }
    // check for unification
    // heavy lifting here is done in the added operator in the literal
    // definition
    if (x == y) {
        return z;
    }
    for (int ix = 0; ix < (x.args).size(); ix++) {
        ...
        else if (boost::apply_visitor(type_visitor(), (y.args).at(ix)) ==
                 "Variable") {
            // substitute y for x
            substitute(y, x, z, ix);
            return unify(x, y, z);
        ...
        // if they are predicates and thus the variable is implicit (hopefully)
        else if (boost::apply_visitor(type_visitor(), (x.args).at(ix)) ==
                     "Predicate" &&
                 boost::apply_visitor(type_visitor(), (y.args).at(ix)) ==
                     "Predicate") {
            // if the arguments are predicates we run them through the
            // unification algorithm again here we now have to construct a
            // Literal data type from the function data type argument of a
            // previous literal to then pass back into the unification algoirthm
            Literal<Term> x1;
            Literal<Term> y1;
            ...
            // the unification function doesnt do the replacements outside the
            // scope of the function, perhaps I remove this argument since its
            // already unified so it doesn't impact the checker?
            sub_list z1;
            z1 = unify(x1, y1, z1);
            // merge z1 and z, since unification could have failed z1 could have
            // a fail condition passed along too concerned
            if (holds_alternative<string>(z1)) {
                return z1;
            }
            else {
                get<sub_list_type>(z).insert(get<sub_list_type>(z1).begin(),
                                             get<sub_list_type>(z1).end());
            }
            // remove the argument
            x.args.erase(x.args.begin() + ix);
    ...
    // if unification algorithm can't sub these literal then it has failed /
    // unification isn't possible
    if (!(x == y)) {
        sub_list z_s; // need to create a new variant sub_list incase it has
                      // already had a sub_list_type filled in
        z_s = "Error: Constants or Predicates do not match. Unification is not "
              "possible.";
        return z_s;
    }
}
