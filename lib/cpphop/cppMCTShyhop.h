...
simulation(std::vector<std::string>& plan,
  ...
  if (state.ask(goal)) {
    ...
    for (auto const& [act,def] : domain_c.actions) {
...
cppMCTShyhop(DomainDef& domain_htn,
           ...
           int seed = 4021,
           std::string const& redis_address = "") {
    domain_htn.set_scorer(scorer_htn);
    domain_c.set_scorer(scorer_c);
    pTree t;
    TaskTree tasktree;
    pNode root;
