...
class ImplicationsOut : FOLVisitor {
  public:
    ...
    auto visitPredicate(Predicate p, auto arg) { return p; }

    auto visitTermEquality(TermEquality equality, auto arg) { return equality; }

    auto visitVariable(Variable variable, auto arg) { return variable; }

    auto visitConstant(Constant constant, auto arg) { return constant; }

    auto visitFunction(Function function, auto arg) { return function; }

    auto visitNotSentence(NotSentence notSentence, auto arg) {
...
class CNFConverter {
  ...
  public:
    CNF convertToCNF(Sentence aSentence) {
        // I)mplications Out:
        Sentence implicationsOut =
            (Sentence)aSentence.accept(new ImplicationsOut(), null);

        // N)egations In:
        Sentence negationsIn =
            (Sentence)implicationsOut.accept(new NegationsIn(), null);
        ...
        // D)istribution
        // V over ^:
        Sentence orDistributedOverAnd =
            (Sentence)andsAndOrs.accept(new DistributeOrOverAnd(), null);
    ...
    auto visitNotSentence(NotSentence notSentence, auto arg) {
        ...
        if (negated instanceof ConnectedSentence) {
            ...
            if (Connectors.isAND(negConnected.getConnector())) {
                ...
                Sentence notAlpha =
                    (Sentence)(new NotSentence(alpha)).accept(this, arg);
                Sentence notBeta =
                    (Sentence)(new NotSentence(beta)).accept(this, arg);
...
class StandardizeQuantiferVariables implements FOLVisitor {
  ...
  private
    StandardizeApartIndexical quantifiedIndexical =
        new StandardizeApartIndexical() {
      private
        int index = 0;

      public
        String getPrefix() { return "q"; }
  ...
  private
    SubstVisitor substVisitor = null;
  ...
  public
    ...
    @SuppressWarnings("unchecked") public auto visitQuantifiedSentence(
        QuantifiedSentence sentence, auto arg) {
        ...
        Sentence subst =
            substVisitor.subst(localSubst, sentence.getQuantified());
        ...
        return new QuantifiedSentence(
            sentence.getQuantifier(), replVariables, sQuantified);
    }
}
...
class RemoveQuantifiers implements FOLVisitor {
  ...
  private
    FOLParser parser = null;
  private
    SubstVisitor substVisitor = null;

  public
    RemoveQuantifiers(FOLParser parser) {
        this.parser = parser;
  ...
  public
    ...
    @SuppressWarnings("unchecked") public auto visitQuantifiedSentence(
        ...
        if (Quantifiers.isEXISTS(sentence.getQuantifier())) {
            Map<Variable, Term> skolemSubst =
                new LinkedHashMap<Variable, Term>();
...
class CNFConstructor implements FOLVisitor {
  ...
  public
    auto visitVariable(Variable variable, auto arg) {
        ...
        throw new IllegalStateException(
            "visitVariable() should not be called.");
    }
  ...
  public
    auto visitConstant(Constant constant, auto arg) {
        // This should not be called
        throw new IllegalStateException(
            "visitConstant() should not be called.");
    }
  ...
  public
    auto visitFunction(Function function, auto arg) {
        // This should not be called
        throw new IllegalStateException(
            "visitFunction() should not be called.");
    }
