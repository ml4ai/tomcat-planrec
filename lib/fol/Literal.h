    ...
    template <class T> struct Literal {
        ...
        friend bool operator==(const Literal<T>& lhs, const Literal<T>& rhs) {
            ...
            for (int i = 0; i < lhs.args.size(); i++) {
                if (boost::apply_visitor(type_visitor(), (lhs.args).at(i)) ==
                        "Variable" ||
                    boost::apply_visitor(type_visitor(), (rhs.args).at(i)) ==
                        "Variable") {
                    eq = false; // any variable means they aren't unified
                }
                else if (boost::apply_visitor(type_visitor(),
                                              (lhs.args).at(i)) !=
                         boost::apply_visitor(type_visitor(),
                                              (rhs.args).at(i))) {
                    eq = false; // different data types
                }
                else if (boost::apply_visitor(type_visitor(),
                                              (lhs.args).at(i)) == "Constant") {
                    if (get<Constant>(lhs.args.at(i)).name !=
                        get<Constant>(rhs.args.at(i)).name) {
                        eq = false; // both entities but different ones
                    }
                }
                else if (boost::apply_visitor(
                             type_visitor(), (lhs.args).at(i)) == "Predicate") {
                    if (get<Function>(lhs.args.at(i)).name !=
                        get<Function>(rhs.args.at(i)).name) {
