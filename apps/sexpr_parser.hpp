...
namespace boost {
    namespace spirit {
        namespace traits {
            ...
            template <>
            struct transform_attribute<utree::nil_type,
                                       unused_type,
                                       qi::domain> {
...
namespace sexpr {
    ...
    struct tagger : qi::grammar<Iterator, void(utree&, char)> {
        qi::rule<Iterator, void(utree&, char)> start;

        qi::rule<Iterator, void(utree&)> epsilon;
...
namespace sexpr {
    ...
    template <typename Iterator,
              typename ErrorHandler = error_handler<Iterator>>
    struct parser : qi::grammar<Iterator, utree(), whitespace<Iterator>> {

        qi::rule<Iterator, utree(), whitespace<Iterator>> start, element, list;
        qi::rule<Iterator, utree()> atom;
        qi::rule<Iterator, int()> integer;
        qi::rule<Iterator, utf8_symbol_type()> symbol;
        qi::rule<Iterator, utree::nil_type()> nil_;
        qi::rule<Iterator, binary_string_type()> binary;
        utf8::parser<Iterator> string;
        ...
        parser(std::string const& source_file = "<string>")
            ...
            atom = nil_ | strict_double | integer | boolean | string | symbol |
                   binary;
