...
bool isValid(
    const Pair& point) { // Returns true if row number and column number is in
                         // range
    return (point.first >= 0) && (point.first < ROW) && (point.second >= 0) &&
           (point.second < COL);
}

// A Utility Function to check whether the given cell is
// blocked or not
bool isUnBlocked(int grid[ROW][COL], const Pair& point) {
    // Returns true if the cell is not blocked else false
    return isValid(point) && grid[point.first][point.second] != 4;
}
...
tuple<int, stack<Pair>> tracePath(const array<array<cell, COL>, ROW>& cellDetails,
               const Pair& dest) {
//    printf("\nThe Path is ");
...
// Algorithm
tuple<int, stack<Pair>> aStarSearch(int grid[ROW][COL], const Pair& src, const Pair& dest) {
...
//        printf("We are already at the destination\n");
    ...
    std::priority_queue<Tuple, std::vector<Tuple>, std::greater<Tuple>>
        openList;
            ...
            for (int add_y = -1; add_y <= 1; add_y++) {
                ...
                if (isValid(neighbour)) {
                    ...
                    if (isDestination(neighbour,
                        ...
                        cellDetails[neighbour.first][neighbour.second]
                            .parent = {i, j};
                    ...
                    // ignore it.  Else do the following
                        ...
                        if (cellDetails[neighbour.first][neighbour.second].f ==
                            ...
                            cellDetails[neighbour.first][neighbour.second].f >
                                fNew) {
                            openList.emplace(
                                fNew, neighbour.first, neighbour.second);
...
tuple<int, stack<Pair>> find_path(Pair src, Pair dest, int map[ROW][COL]) {
