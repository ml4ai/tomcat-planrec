...
template <class State, class Domain, class Selector>
std::pair<Tree<State, Selector>, int> expansion(Tree<State,Selector> t, Domain domain,Selector selector, int v, int seed = 2021) {
...
Tree<State,Selector> seek_planMCTS(Tree<State,Selector> t,
  ...
  for (int i = 0; i < r; ++i) {
    ...
    double results = simulation(cState,
                                cTasks,
                                domain,
                                selector,
                                ++seed);
    mp.first[mp.second].selector.mean = results;
    mp.first[mp.second].selector.sims = mp.first[mp.second].selector.sims + 1;
