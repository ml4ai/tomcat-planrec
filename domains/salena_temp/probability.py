# TODO Start on line 170 for work done and to be done for the final paper
    # TODO  for the directed research. These files in the PDDL personal file
    # TODO are the most recent versions and really need to be rewritten
    # TODO  4 August 2022     Salena
...
SUPER_REPLACED_DATAFRAME = False
#--------------------------------------------------------------------------
...
def get_conditional_probability(df, colA, colB):
    ...
    df1 = df1[["abstractLabels", "questionLabels", "conditional_probability"]]
#    print("\n Conditional Probability \t P(", colB, "|", colA, ")\n")
    ...
    """
    As of 4 August 2022, these next few lines will be my go-to for finishing
    my final for the directed research.

    Honestly, I need to condense a lot of these files into one utility file.
    """

    ### From these conditional probabilities, filter by word of interest:
    #myInterest = "carryStabilized"
    myInterest = "carryStabilized"
    req = df1['abstractLabels'].str.contains(myInterest, case = False, na=False)
    #req = df1['questionLabels']== myInterest
    req = df1[req]
    get_joint_probability(req, "questionLabels")
    print(req.sort_values(by=['questionLabels']))
...
def replaceTerms(df):
    ...
    df = replaceSubstring_Global(df, "ii", "i")
...
def super_replace(df):
    ...
    print("\n\nSuper Replace\n\n")
    time.sleep(2)
    ### Super-replace terms for minimal granularity
    df = replaceSubstring_Global(df, "request", "ask")
    ...
    df = replaceSubstring_Global(df, "confirm", "tell")
    df = replaceSubstring_Global(df, "Stabilized", "Saved")
    df = replaceSubstring_Global(df, "stabilize", "save")
    df = replaceSubstring_Global(df, "Stabilize", "Save")
    df = replaceSubstring_Global(df, "rescue", "save")
    df = replaceSubstring_Global(df, "Rescue", "Save")
    df = replaceSubstring_Global(df, "Destination", "Location")
    df = replaceSubstring_Global(df, "Navigation", "Location")
    df = replaceSubstring_Global(df, "Navigate", "Location")
    df = replaceSubstring_Global(df, "room", "room")
    df = replaceSubstring_Global(df, "Search", "Clear")
    df = replaceSubstring_Global(df, "krR", "kR")
    df = replaceSubstring_Global(df, "Blocked", "Rubble")
    df = replaceSubstring_Global(df, "informLocation", "askLocation")
    df = replaceSubstring_Global(df, "askDestination", "askLocation")
    df = replaceSubstring_Global(df, "askCarryDrop", "askCarryVictim")
    df = replaceSubstring_Global(df, "LocationLocation", "Location")
    df = replaceSubstring_Global(df, "Stabized", "Saved")
#    df = replaceSubstring_Global(df, "askCriticalStatus", "askStatus")
    df = replaceSubstring_Global(df, "askStatusVictim", "askStatus")
    df = replaceSubstring_Global(df, "MarkLocation", "LocationMark")
    df = replaceSubstring_Global(df, "RubbleLocation", "LocationRubble")
    df = replaceSubstring_Global(df, "askThreatroomroom", "askThreatroom")
#    df = replaceSubstring_Global(df, "ask", "")
#    df = replaceSubstring_Global(df, "clarify", "")
#    df = replaceSubstring_Global(df, "collaborate", "")
#    df = replaceSubstring_Global(df, "prioritize", "")

    reclean(df, "abstractLabels", "abstractLabels")
    reclean(df, "questionLabels", "questionLabels")
...
if __name__ == '__main__':
    ...
    if REPLACED_DATAFRAME:
        get_joint_probability(r_dataframe, "questionLabels")
        get_conditional_probability(r_dataframe, "questionLabels", "abstractLabels")

    if SUPER_REPLACED_DATAFRAME:
        s_dataframe = super_replace(r_dataframe)
        s_dataframe = pandas.DataFrame(s_dataframe)
        get_conditional_probability(s_dataframe, "questionLabels", "abstractLabels")
        superReplaced_file = ("../data/super_replacedHSR_readyToUse.csv")
        s_dataframe.to_csv(superReplaced_file)
